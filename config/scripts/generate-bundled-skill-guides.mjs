...
// Why: the desktop catalog needs name + description only, never the embedded
// markdown, so it ships as its own small table instead of importing the CLI's.
function serializeSharedCatalog(guides) {
  const entries = guides
    .map(
      (guide) =>
        `  { name: ${JSON.stringify(guide.name)}, description: ${JSON.stringify(guide.description)} }`
    )
    .join(',\n')
  return `// Generated by config/scripts/generate-bundled-skill-guides.mjs. Do not edit.\n\nexport type BundledSkillCatalogEntry = {\n  readonly name: string\n  readonly description: string\n}\n\n// oxfmt-ignore\nexport const BUNDLED_SKILL_CATALOG = [\n${entries}\n] as const satisfies readonly BundledSkillCatalogEntry[]\n`
}
...
async function buildArtifacts(repoRoot = REPO_ROOT) {
  ...
  return [
    ...
    {
      path: path.join(repoRoot, 'src', 'shared', 'bundled-skill-catalog.ts'),
      content: serializeSharedCatalog(guides)
    },
    ...projections
  ]
}
