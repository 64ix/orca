...
describe('setAgentStatuses', () => {
  ...
  it('matches sequential updates across repeated panes and provider-session records', async () => {
    ...
    let transientCompletionOutput: string | undefined
    batchStore.subscribe((next) => {
      const entry = next.agentStatusByPaneKey[FIRST_PANE]
      if (entry) {
        transientCompletionOutput = next
          .getTransientAgentStatusTransitions(FIRST_PANE)
          .find((transition) => transition.state === 'done')?.lastAssistantMessage
      }
    })
  ...
  it('shares freshness across nested folds but isolates synchronous reentrant transactions', async () => {
    ...
    let reenteredBeforeThrow = false
    const unsubscribeThrowing = store.subscribe((next) => {
      if (reenteredBeforeThrow) {
        return
      }
      reenteredBeforeThrow = true
      next.setAgentStatuses([update(SECOND_PANE, BASE_TIME + 5)])
      throw new Error('subscriber failed')
    })
    expect(() => store.getState().setAgentStatuses([update(FIRST_PANE, BASE_TIME + 4)])).toThrow(
      'subscriber failed'
    )
    unsubscribeThrowing()
    expect(store.getState().getTransientAgentStatusTransitions(FIRST_PANE)).toEqual([])
    expect(store.getState().getTransientAgentStatusTransitions(SECOND_PANE)).toEqual([])
    await flushMicrotasks()
  })
  ...
  it('hides the ledger from publications after the status commit', () => {
    const store = seedBatchStore()
    const observedStates: string[][] = []
    store.subscribe((next) => {
      observedStates.push(
        next.getTransientAgentStatusTransitions(FIRST_PANE).map((entry) => entry.state)
      )
    })

    store.getState().transactAgentStatuses((transaction) => {
      transaction.apply({
        paneKey: FIRST_PANE,
        payload: { state: 'working', prompt: 'turn', agentType: 'pi' },
        timing: { updatedAt: BASE_TIME, stateStartedAt: BASE_TIME }
      })
      transaction.afterCommit(() => store.setState({ activeWorktreeId: 'after-commit' }))
    })

    expect(observedStates).toEqual([['working'], []])
  })

  it('exposes every ordered completion beyond the display cap only during publication', async () => {
    const store = seedBatchStore()
      ...
      if (entry) {
        observedOutputs.push(
          ...next
            .getTransientAgentStatusTransitions(FIRST_PANE)
            .flatMap((transition) =>
              transition.state === 'done' && transition.lastAssistantMessage
                ? [transition.lastAssistantMessage]
                : []
            )
        )
      }
    ...
    expect(observedOutputs).toEqual(Array.from({ length: 25 }, (_, turn) => `output ${turn}`))
    expect(store.getState().getTransientAgentStatusTransitions(FIRST_PANE)).toEqual([])
    expect(entry.lastAssistantMessage).toBe('output 24')
    await flushMicrotasks()
    expect(store.getState().refreshGitHubForWorktreeIfStale).toHaveBeenCalledOnce()
  })

  it('keeps completion edges visible when provider-only cleanup removes the live row', () => {
    const store = seedBatchStore()
    const observedTransitions: AgentStatusEntry[] = []
    store.subscribe((next) => {
      observedTransitions.push(...next.getTransientAgentStatusTransitions(FIRST_PANE))
    })
    ...
    store.getState().setAgentStatuses([
      ...
      },
      {
        ...
        agent: 'pi',
        providerSession: PROVIDER_SESSION,
        timing: { updatedAt: BASE_TIME + 2 },
        routing: { tabId: 'tab-1', worktreeId: 'wt-1' }
      }
    ])

    expect(store.getState().agentStatusByPaneKey[FIRST_PANE]).toBeUndefined()
    expect(observedTransitions.map((entry) => entry.state)).toEqual(['working', 'done'])
    expect(observedTransitions.at(-1)?.lastAssistantMessage).toBe('Finished.')
    expect(store.getState().getTransientAgentStatusTransitions(FIRST_PANE)).toEqual([])
  })
