...
import {
  ...
  type AgentHookCompletionNotificationObservation
...
export function useIpcEvents(): void {
  useEffect(() => {
    ...
    type AgentStatusBatchContext = {
      ...
      notificationObservations: AgentHookCompletionNotificationObservation[]
    }
    ...
    function flushPendingAgentStatuses(): void {
      ...
      try {
        ...
        const results = applyAgentStatusBatch(
    ...
    const applyAgentStatus = (
      ...
      const notificationObservation: AgentHookCompletionNotificationObservation | null =
        options?.replay !== true && statusWorktreeId
          ? {
              paneKey,
              worktreeId: statusWorktreeId,
              payload:
      ...
      if (options?.batch) {
        ...
        if (notificationObservation) {
          options.batch.notificationObservations.push(notificationObservation)
        }
        if (
          terminalTitle &&
        ...
        if (notificationObservation) {
          observeAgentHookCompletionForNotification(notificationObservation)
        }
      }
      return 'applied'
    }
    ...
    function applyAgentStatusBatch(
      ...
      return useAppStore.getState().transactAgentStatuses((transaction) => {
        ...
        if (batch.notificationObservations.length > 0) {
          transaction.afterCommit(() =>
            observeAgentHookCompletionsForNotification(batch.notificationObservations)
          )
        }
        return results
      })
