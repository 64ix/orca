...
describe('getSmartGitHubSubmitResolution', () => {
  it('uses the resolved PR title for workspace name, display name, and linked PR metadata', () => {
    expect(
      ...
      workspaceName: 'pr-2049-fix-smart-resolution-delay',
      displayName: 'PR 2049 Fix smart resolution delay',
      linkedWorkItem: {
        type: 'pr',
        number: 2049,
        title: 'Fix smart resolution delay',
  ...
  it('strips duplicated issue prefixes while preserving linked issue metadata', () => {
    ...
    expect(resolution.workspaceName).toBe('issue-2050-make-create-feel-instant')
    expect(resolution.displayName).toBe('Issue 2050 Make create feel instant')
  ...
  it('uses the URL path to normalize stale PR-typed issue results', () => {
    expect(
      ...
      workspaceName: 'issue-6933-the-board-columns-are-displayed-backw',
      displayName: 'Issue 6933 The board columns are displayed backwards',
      linkedWorkItem: {
        type: 'issue',
        number: 6933,
        title: 'The board columns are displayed backwards',
