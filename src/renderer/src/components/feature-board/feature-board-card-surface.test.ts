...
describe('featureBoardCardSurfaceClass', () => {
  it('keeps the card outline and its own surface visible at rest', () => {
    const idle = featureBoardCardSurfaceClass({ isAwaitingInput: false, isSelected: false })
  ...
  it('insets the content so a selection ring never crosses the title', () => {
    ...
    expect(featureBoardCardSurfaceClass({ isAwaitingInput: false, isSelected: false })).toContain(
      'pt-1'
    )
  })

  it('states awaiting-input and selection as ring tints layered over the same outline', () => {
    const awaiting = featureBoardCardSurfaceClass({ isAwaitingInput: true, isSelected: false })
    const selected = featureBoardCardSurfaceClass({ isAwaitingInput: false, isSelected: true })
  ...
  it('lets selection win over awaiting-input so one card never shows two rings', () => {
    const both = featureBoardCardSurfaceClass({ isAwaitingInput: true, isSelected: true })
