...
type UsePanePtyLayoutBindingsParams = {
  tabId: string
...
/**
 * Persists live pane→PTY bindings into the tab's saved layout: binds on attach, clears on
 * close/restart, repairs the active leaf when a focused pane loses its PTY unexpectedly.
 */
export function usePanePtyLayoutBindings({
  tabId,
  setTabLayout,
  managerRef
}: UsePanePtyLayoutBindingsParams) {
